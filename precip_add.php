...
if ($actionGet=="new" || $actionGet=="edit" || !$actionGet)
{
    if ($actionGet=="edit" && $_GET['pm_id'])
    {
        ...
        list ($shr,$smin,$ssec)= explode(":",$stime);
    } elseif ($actionGet == "new" || !$actionGet) {
        $pm_id = $pmdate = $sdate = $stime = $inches = $air_temp_f = $wind_speed_mph = $wind_dir = $pressure_mmhg = $ps_id = null;
    }
    ?>
    <form action="precip_add.php" method="POST">
    <input type="hidden" name="action" value="<?php print $actionGet ? $actionGet :"new";?>">
    <?php 
        if($actionGet=="edit") {print "<input type=\"hidden\" name=\"pm_id\" value=\"$pm_id\">\n";}
    ?>
    <h2>New Precipitation Data</h2>
