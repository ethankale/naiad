...
// If adding or editing a precipitation station, execute the
//  sql based on the provided parameters.

if ($submitPost=="Submit" && ($actionPost=="new" || $actionPost=="edit"))
{
    $throw_error=false;
    if($actionPost=="new") 
    {
    ...
    else print "<p>Monitoring site $StationID $action_result</p>";
}

if ($actionGet=="new" || $actionGet=="edit")
{
    $action_descriptor = "New";
    if ($actionGet=="edit" && $_GET['StationID'])
    {
        ...
        $StationID = $Station_Name = $Description = $latitude = $longitude = null;
    }
    ?>
...
<h1>Precipitation Station</h1>

<table width="500px" class="listtable">
<tbody><tr><td colspan=3><a href="precip_sites.php?action=new">New Precipitation Station</a></td></tr></tbody>
